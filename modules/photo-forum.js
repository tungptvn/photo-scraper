...
function getUserConfig() {
    ...
    return JSON.parse(searchParams.get('userConfig'))
}
...
async function run() {
    var ret = await Promise.all(Array.from(Array(getUserConfig().pTo - getUserConfig().pFrom).keys())
        .map(x => 'https://cors-anywhere.herokuapp.com/' + getUserConfig().url + getUserConfig().pageTemplate.replace('__pageNum__', x + getUserConfig().pFrom + 1))
